...
def _title_case_display(s: str) -> str:
    """Convert raw enum-like strings into a friendlier UI label."""
    s = (s or "").strip()
    s = s.replace("_", " ")
...
def _build_choices_and_map(items: Optional[List[str]]) -> Tuple[List[str], Dict[str, str]]:
    """Return display labels plus a reverse map back to the raw model values."""
    if not items:
        return [], {}
...
def _dtype_from_str(s: str) -> torch.dtype:
    """Parse common dtype aliases into ``torch.dtype`` objects."""
    s = (s or "").strip().lower()
...
def _normalize_audio(wav, eps=1e-12, clip=True):
    """
    Normalize Gradio-provided audio arrays into mono float32 ``[-1, 1]``.

    Gradio may hand back integer PCM arrays or floating-point arrays depending
    on browser/runtime details, so the demo normalizes them before sending them
    into the inference wrapper.
    """
    x = np.asarray(wav)
...
def _parse_audio_any(audio: Any) -> Union[str, Tuple[np.ndarray, int]]:
    """Validate that the UI produced a supported audio payload."""
    if audio is None:
...
def build_parser() -> argparse.ArgumentParser:
    """Create the CLI parser used to launch the Gradio demo."""
...
def _parse_json_dict(s: Optional[str], *, name: str) -> Dict[str, Any]:
    """Parse a JSON object argument and raise a CLI-friendly error on failure."""
...
def _apply_cuda_visible_devices(cuda_visible_devices: str) -> None:
    """Set ``CUDA_VISIBLE_DEVICES`` before importing or constructing GPU objects."""
...
def _default_backend_kwargs(backend: str) -> Dict[str, Any]:
    """Return conservative default kwargs for the selected backend."""
...
def _default_aligner_kwargs() -> Dict[str, Any]:
    """Return the default forced-aligner loading kwargs for the demo."""
    return dict(
        dtype=torch.bfloat16,
        device_map="cuda:0",
    )
...
def _merge_dicts(base: Dict[str, Any], override: Dict[str, Any]) -> Dict[str, Any]:
    """Shallow-merge two dictionaries without mutating either input."""
    out = dict(base)
    out.update(override)
    return out
...
def _coerce_special_types(d: Dict[str, Any]) -> Dict[str, Any]:
    """Convert JSON-friendly strings such as ``\"bfloat16\"`` into Python objects."""
    out: Dict[str, Any] = {}
    for k, v in d.items():
...
def build_demo(
    ...
    """
    Build the Gradio interface around a preloaded ASR wrapper.

    The UI deliberately stays thin: all audio normalization and inference logic
...
def main(argv=None) -> int:
    """Load the requested backend/checkpoints and launch the Gradio server."""
    parser = build_parser()
