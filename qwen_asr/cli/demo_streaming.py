...
@dataclass
class Session:
    """Server-side state for one browser streaming session."""
...
def _gc_sessions():
    """Expire inactive sessions and flush their remaining audio buffers."""
    now = time.time()
...
def _get_session(session_id: str) -> Optional[Session]:
    """Fetch a session and update its liveness timestamp."""
    _gc_sessions()
...
# Keeping the HTML/JS inline makes the demo easy to launch as a single Python
# file with no frontend build step.
...
@app.get("/")
def index():
    """Serve the single-page streaming demo."""
    return Response(INDEX_HTML, mimetype="text/html; charset=utf-8")


@app.post("/api/start")
def api_start():
    """Create a fresh streaming decoder state and return its session id."""
...
@app.post("/api/chunk")
def api_chunk():
    """Accept one float32 audio chunk and return the latest partial transcript."""
...
@app.post("/api/finish")
def api_finish():
    """Flush tail audio, finalize decoding and delete the session."""
    session_id = request.args.get("session_id", "")
    s = _get_session(session_id)
    if not s:
        return jsonify({"error": "invalid session_id"}), 400
...
def main():
    """Load the vLLM backend once and start the Flask development server."""
    args = parse_args()
