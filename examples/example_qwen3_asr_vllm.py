...
"""
Examples for Qwen3ASRModel using the vLLM backend.

Compared with the Transformers example, this version highlights the production
inference path:

1. Faster autoregressive decoding through vLLM scheduling and KV-cache
   management.
2. The same flexible input surface at the Python API level.
3. Optional timestamp recovery by attaching the forced aligner wrapper.
"""
...
def _download_audio_bytes(url: str, timeout: int = 30) -> bytes:
    """Fetch a demo audio clip from a URL."""
...
def _read_wav_from_bytes(audio_bytes: bytes) -> Tuple[np.ndarray, int]:
    """Decode bytes into ``(waveform, sample_rate)`` using ``soundfile``."""
...
def _to_data_url_base64(audio_bytes: bytes, mime: str = "audio/wav") -> str:
    """Convert raw audio bytes into a base64 data URL."""
    b64 = base64.b64encode(audio_bytes).decode("utf-8")
    return f"data:{mime};base64,{b64}"


def _print_result(title: str, results) -> None:
    """Print a compact summary of recognition outputs."""
    print(f"\n===== {title} =====")
...
def test_single_url(asr: Qwen3ASRModel) -> None:
    """Run one vLLM-backed transcription from a URL input."""
...
def test_batch_mixed(asr: Qwen3ASRModel) -> None:
    """Demonstrate mixed URL/base64/numpy audio inputs in one batch."""
...
def test_single_with_timestamps(asr: Qwen3ASRModel) -> None:
    """Use vLLM ASR plus the forced aligner for timestamped output."""
    results = asr.transcribe(
        audio=URL_EN,
        language="English",
...
def test_batch_with_timestamps(asr: Qwen3ASRModel) -> None:
    """Run batch recognition with timestamps and explicit language prompts."""
    zh_bytes = _download_audio_bytes(URL_ZH)
    zh_b64 = _to_data_url_base64(zh_bytes, mime="audio/wav")
...
def main() -> None:
    """Instantiate the vLLM backend and execute every example case."""
