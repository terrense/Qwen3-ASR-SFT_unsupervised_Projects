...
def _download_audio_bytes(url: str, timeout: int = 30) -> bytes:
    """Fetch one demo utterance for the streaming example."""
...
def _read_wav_from_bytes(audio_bytes: bytes) -> Tuple[np.ndarray, int]:
    """Decode a byte buffer into waveform samples and its sample rate."""
...
def run_streaming_case(asr: Qwen3ASRModel, wav16k: np.ndarray, step_ms: int) -> None:
    """
    Feed a waveform to the streaming API in fixed-size wall-clock steps.

    This mirrors how a browser or microphone client would progressively send
    audio frames to the backend instead of waiting for the full utterance.
    """
    sr = 16000
