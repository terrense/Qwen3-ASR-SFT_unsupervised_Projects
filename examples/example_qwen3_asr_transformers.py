...
def _download_audio_bytes(url: str, timeout: int = 30) -> bytes:
    """Fetch a demo audio clip from the public repository assets."""
...
def _read_wav_from_bytes(audio_bytes: bytes) -> Tuple[np.ndarray, int]:
    """Decode in-memory audio bytes into ``(waveform, sample_rate)``."""
...
def _to_data_url_base64(audio_bytes: bytes, mime: str = "audio/wav") -> str:
    """Convert raw audio bytes into a browser-style base64 data URL."""
    b64 = base64.b64encode(audio_bytes).decode("utf-8")
    return f"data:{mime};base64,{b64}"


def _print_result(title: str, results) -> None:
    """Print a compact summary of the returned ASR results."""
    print(f"\n===== {title} =====")
...
def test_single_url(asr: Qwen3ASRModel) -> None:
    """Recognize one utterance from an HTTPS URL without timestamps."""
...
def test_batch_mixed(asr: Qwen3ASRModel) -> None:
    """Show that the wrapper accepts mixed transport formats in one batch."""
...
def test_single_with_timestamps(asr: Qwen3ASRModel) -> None:
    """Recognize one utterance and recover token timestamps via forced alignment."""
    results = asr.transcribe(
        audio=URL_EN,
        language="English",
...
def test_batch_with_timestamps(asr: Qwen3ASRModel) -> None:
    """Run batch ASR with explicit languages and timestamp output enabled."""
    zh_bytes = _download_audio_bytes(URL_ZH)
    zh_b64 = _to_data_url_base64(zh_bytes, mime="audio/wav")
...
def main() -> None:
    """Instantiate the Transformers backend and run all demo cases."""
